...
public class TestController {
    ...
    @GetMapping
    public ApiResponse<List<CoreDtos.TestResponse>> all() {
        return ApiResponse.ok(service.tests());
    }

    @GetMapping("/{id}")
    public ApiResponse<CoreDtos.TestResponse> one(@PathVariable Long id) {
        return ApiResponse.ok(service.test(id));
    }

    @PostMapping
    @PreAuthorize("hasRole('ADMIN')")
    public ApiResponse<CoreDtos.TestResponse> create(@Valid @RequestBody CoreDtos.TestRequest request) {
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ApiResponse<CoreDtos.TestResponse> update(@PathVariable Long id,
            @Valid @RequestBody CoreDtos.TestRequest request) {
        return ApiResponse.ok(service.updateTest(id, request));
    }
